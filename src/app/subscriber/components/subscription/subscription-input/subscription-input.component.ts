...
export class SubscriptionInputComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.editMode) {
      ...
      this.subscriptionService.editSubscription(this.subscription).subscribe((result) => {
        this.dialogRef.close({event: 'Edit'});
      })
    } else {
      this.subscriptionService.addSubscription(this.subscription).subscribe((result) => {
  ...
  getPublications() {
    this.publicationService.getPublications().subscribe((result) => {
      ...
      if (this.editMode) {
        this.subscriptionForm.patchValue({
          publication: this.publicationOptions.find(publication => publication.id == this.data.subscription.publicationId),
        });
      }
    })
  }
  ...
  getSubscriptionTypes() {
    this.subscriptionTypeService.getSubscriptionTypes().subscribe((result) => {
      ...
      if (this.editMode) {
        this.subscriptionForm.patchValue({
          subscriptionType: this.typeOptions.find(type => type.id == this.data.subscription.subscriptionTypeId),
        });
      }
    })
  }
