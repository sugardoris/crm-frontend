...
import {Publication, PublicationPeriod, PublishingInfo} from "../../../domain/publication";
...
export class PublicationInputComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: {publication: Publication, mode: string}
  ) { }
  ...
  getPublicationData(publication: Publication) {
    let issueDate = new Date(this.data.publication.publishingInfo.firstIssueDate);
    console.log(issueDate);
    if (this.data.publication) {
      this.publicationForm.patchValue({
        name: this.data.publication.name,
        firstIssue: new Date(this.data.publication.publishingInfo.firstIssueDate),
        issuePeriod: this.data.publication.publishingInfo.issuePeriod,
        price: this.data.publication.publishingInfo.price,
        comesOut: this.data.publication.publishingInfo.comesOut
      });
    }
  }
